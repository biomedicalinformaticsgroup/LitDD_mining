...
## Headline (deployed corpus, all-papers denominator, min_year 1981)
...
~1–4 points, so the filter's recall cost is small. The dominant miss is `not_in_corpus` —
curated PMIDs the BERT screen never saw (largely out-of-scope non-case-report citations),
the R3.4 characterisation. Outputs go to the gitignored `revision/external_recall/`.
